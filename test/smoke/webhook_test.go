...
func TestWebhook_MatchConditions_PerCluster(t *testing.T) {
	etcd := os.Getenv("ETCD_ENDPOINTS")
	s := startAPIServer(t, etcd)
	...
	// Service + EndpointSlice that resolves to our local webhook server.
	svcName := "svc-wh-mc-" + randSuffix(3)
	serviceDNS := svcName + "." + ns + ".svc"
	...
	// ValidatingWebhookConfiguration in clusterA that only matches a specific name.
	whName := "vwh-mc-" + randSuffix(3)
	sideEffects := admissionregv1.SideEffectClassNone
	failurePolicy := admissionregv1.Fail
	matchPolicy := admissionregv1.Equivalent
	timeout := int32(5)
	...
	path := "/validate"
	port := int32(443)
	matchExpression := `object.metadata.name == "deny-cm"`
	_, err = csA.AdmissionregistrationV1().ValidatingWebhookConfigurations().Create(ctx, &admissionregv1.ValidatingWebhookConfiguration{
		ObjectMeta: metav1.ObjectMeta{Name: whName},
		Webhooks: []admissionregv1.ValidatingWebhook{
			{
				Name: "match-conditions.smoke.kplane.dev",
				ClientConfig: admissionregv1.WebhookClientConfig{
					Service: &admissionregv1.ServiceReference{
						Namespace: ns,
						Name:      svcName,
				...
				MatchConditions: []admissionregv1.MatchCondition{
					{Name: "deny-by-name", Expression: matchExpression},
				},
				FailurePolicy:           &failurePolicy,
				MatchPolicy:             &matchPolicy,
				SideEffects:             &sideEffects,
	...
	// Should succeed in clusterA because matchConditions do not match.
	_, err = csA.CoreV1().ConfigMaps(ns).Create(ctx, &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "allow-cm"},
	}, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("cluster=%s expected allow-cm create success, got: %v", clusterA, err)
	}

	// Should be denied in clusterA because matchConditions match.
	_, err = csA.CoreV1().ConfigMaps(ns).Create(ctx, &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "deny-cm"},
	}, metav1.CreateOptions{})
	if err == nil || !apierrors.IsForbidden(err) {
		t.Fatalf("cluster=%s expected deny-cm to be forbidden, got: %v", clusterA, err)
	}

	// ClusterB should not be affected by clusterA's webhook.
	_, err = csB.CoreV1().ConfigMaps(ns).Create(ctx, &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "deny-cm"},
	}, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("cluster=%s expected deny-cm create success, got: %v", clusterB, err)
	}
}
