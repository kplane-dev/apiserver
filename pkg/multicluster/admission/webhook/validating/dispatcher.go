		...
		go func(invocation *generic.WebhookInvocation, idx int) {
			...
			if callErr, ok := err.(*webhookutil.ErrCallingWebhook); ok {
				if ignoreClientCallFailures {
					...
					case <-ctx.Done():
						// parent context is canceled or timed out, no point in continuing
						errCh <- apierrors.NewTimeoutError("request did not complete within requested timeout", 0)
					default:
			...
			if callErr, ok := err.(*webhookutil.ErrCallingWebhook); ok {
				klog.Warningf("Failed calling webhook, failing closed %v: %v", hook.Name, callErr)
				errCh <- callErr
				return
			}

			if rejectionErr, ok := err.(*webhookutil.ErrWebhookRejection); ok {
				errCh <- rejectionErr.Status
				return
			}
		}(relevantHooks[i], i)
	}
...
func (d *validatingDispatcher) callHook(ctx context.Context, h *v1.ValidatingWebhook, invocation *generic.WebhookInvocation, versionedAttr *admission.VersionedAttributes) error {
	if versionedAttr.Attributes.IsDryRun() {
	...
	ctx, span := tracing.Start(ctx, "Call validating webhook",
		...
		attribute.Stringer("resource", versionedAttr.GetResource()),
		attribute.String("subresource", versionedAttr.GetSubresource()),
		attribute.String("operation", string(versionedAttr.GetOperation())),
